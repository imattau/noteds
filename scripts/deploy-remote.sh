...
SKIP_BUILD=false
SSH_TARGET=""
SSH_CONTROL_PATH="${TMPDIR:-/tmp}/noteds-ssh-%C"
...
Options:
  ...
  --install-dir <path>     Remote install path (default: /var/www/noteds)
...
Environment overrides:
  NOTEDS_SSH_TARGET, NOTEDS_PORT, NOTEDS_INSTALL_DIR,
  NOTEDS_SERVICE_USER, NOTEDS_SERVICE_GROUP, NOTEDS_PROXY,
  NOTEDS_DOMAIN, NOTEDS_CADDY_EMAIL, NOTEDS_SSH_PORT,
  NOTEDS_DRY_RUN, NOTEDS_SKIP_BUILD
EOF
}
...
managed_marker="# managed by noteds deploy"
...
printf '\n'
hr
panel_line "noteds remote installer"
hr
panel_line "install  : ${INSTALL_DIR}"
panel_line "service  : ${SERVICE_NAME}"
panel_line "port     : ${PORT}"
...
detect_caddy_snippet_dir() {
	...
	local dir
	if [[ -n "${NOTEDS_CADDY_SNIPPET_DIR:-}" ]]; then
		printf '%s' "$NOTEDS_CADDY_SNIPPET_DIR"
		return 0
	fi
	if [[ -f "$main_file" ]]; then
...
${import_line}"
		...
		if grep -qE '^[[:space:]]*import[[:space:]].*(/etc/caddy/)?(conf\.d|Caddyfile\.d)/\*\.caddy' "$main_file"; then
			...
			log "existing Caddyfile is managed by noteds; adding snippet import"
...
detect_nginx_style() {
	...
	if [[ -n "${NOTEDS_NGINX_STYLE:-}" ]]; then
		printf '%s' "$NOTEDS_NGINX_STYLE"
		return 0
	fi
...
parse_env() {
	SSH_TARGET="${NOTEDS_SSH_TARGET:-${SSH_TARGET:-}}"
	PORT="${NOTEDS_PORT:-${PORT:-3000}}"
	INSTALL_DIR="${NOTEDS_INSTALL_DIR:-${INSTALL_DIR:-/var/www/noteds}}"
	SERVICE_USER="${NOTEDS_SERVICE_USER:-${SERVICE_USER:-www-data}}"
	SERVICE_GROUP="${NOTEDS_SERVICE_GROUP:-${SERVICE_GROUP:-www-data}}"
	PROXY_MODE="${NOTEDS_PROXY:-${PROXY_MODE:-auto}}"
	DOMAIN="${NOTEDS_DOMAIN:-${DOMAIN:-}}"
	CADDY_EMAIL="${NOTEDS_CADDY_EMAIL:-${CADDY_EMAIL:-}}"
	SSH_PORT="${NOTEDS_SSH_PORT:-${SSH_PORT:-22}}"
	if [[ -n "${NOTEDS_DRY_RUN:-}" ]] && is_true "${NOTEDS_DRY_RUN}"; then
		DRY_RUN=true
	fi
	if [[ -n "${NOTEDS_SKIP_BUILD:-}" ]] && is_true "${NOTEDS_SKIP_BUILD}"; then
		SKIP_BUILD=true
	fi
}
